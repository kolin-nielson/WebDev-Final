...
function createplat() {
  for (i = 0; i < num; i++) {
    platforms.push({
      x: Radomplat(4, 100) * i,
      y: 200 + Radomplat(1, 100) * i,
      width: Radomplat(80, 200),
      height: 15,
    });
  }
}
...
function loop() {
  ...
  for (let k = 0; k < num; k++) {
    if (
      platforms[k].x <= player.x &&
      player.x <= platforms[k].x + platforms[k].width &&
      platforms[k].y <= player.y &&
      player.y <= platforms[k].y + platforms[k].height
    ) {
      i = k;
    }
  }
